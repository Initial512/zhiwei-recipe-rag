...
class GraphIndexingModule:
    ...
    def create_entity_key_values(
        ...
        for recipe in recipes:
            ...
            entity_kv = EntityKeyValue(
                ...
                value_content="\n".join(content_parts),
                entity_type="Recipe",
                metadata={"node_id": entity_id, "properties": getattr(recipe, "properties", {})},
            )
        ...
        for step in cooking_steps:
            ...
            entity_kv = EntityKeyValue(
                ...
                metadata={"node_id": entity_id, "properties": getattr(step, "properties", {})},
            )
    ...
    def _generate_relation_index_keys(
        ...
        if getattr(self.config, "enable_llm_relation_keys", False):
            enhanced_keys = self._llm_enhance_relation_keys(
                source_entity, target_entity, relation_type
            )
    ...
    def deduplicate_entities_and_relations(self):
        ...
        logger.info(
            f"去重完成 - 删除了 {len(entities_to_remove)} 个重复实体，{len(relations_to_remove)} 个重复关系"
        )
