...
分类结果:""")
    ...
    def classify_query_scope(self, query: str) -> str:
        """判断查询应进入菜谱检索还是饮食助手。"""
        prompt = ChatPromptTemplate.from_template("""
你是“知味 AI”的问题分类器。请将用户问题严格分类为以下一种：

- recipe：用户在询问具体菜品、菜谱推荐、食材、做法、步骤、口味菜品或餐食选择。
- assistant：用户在询问饮食常识、饮食文化、知味 AI 的身份或能力，或者问题与饮食无关。

只返回 recipe 或 assistant，不要解释。
...
分类：""")
    ...
    def generate_assistant_answer_stream(self, query: str):
        """生成不使用菜谱检索上下文的饮食助手回答。"""
        prompt = ChatPromptTemplate.from_template("""
你是“知味 AI”，一位专注饮食、烹饪与菜谱服务的中文 AI 助手。

回答规则：
1. 可以介绍你是谁、你能做什么。
2. 可以回答饮食常识、食材知识、烹饪文化和饮食相关问题。
